...
func (p *Parser) getTokenBalanceChanges(mint solana.PublicKey) (int64, error) {
	if mint == NATIVE_SOL_MINT_PROGRAM_ID {
		...
		if len(p.meta.PostBalances) == 0 || len(p.meta.PreBalances) == 0 {
			return 0, fmt.Errorf("insufficient balance information for SOL")
		}
		change := int64(p.meta.PostBalances[0]) - int64(p.meta.PreBalances[0])
		return change, nil
	}
	...
	for _, preBalance := range p.meta.PreTokenBalances {
	...
	for _, postBalance := range p.meta.PostTokenBalances {
